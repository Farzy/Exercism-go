...
func TestSumOfSquaresSimple(t *testing.T) {
	for _, test := range tests {
		t.Run(fmt.Sprintf("Sum of squares from 1 to %d", test.input), func(t *testing.T) {
			if got := SumOfSquaresSimple(test.input); got != test.sumOfSquares {
				t.Fatalf("SumOfSquaresSimple(%d) = %d, want: %d", test.input, got, test.sumOfSquares)
			}
		})
	}
}
...
func TestDifferenceSimple(t *testing.T) {
	for _, test := range tests {
		t.Run(fmt.Sprintf("Difference of SquareOfSum and SumOfSquares of value %d", test.input), func(t *testing.T) {
			want := test.squareOfSum - test.sumOfSquares
			if got := DifferenceSimple(test.input); got != want {
				t.Fatalf("DifferenceSimple(%d) = %d, want: %d", test.input, got, want)
			}
		})
	}
}
...
func BenchmarkSumOfSquaresSimple(b *testing.B) {
	if testing.Short() {
		b.Skip("skipping benchmark in short mode.")
	}
	for i := 0; i < b.N; i++ {
		SumOfSquaresSimple(100)
	}
}

func BenchmarkDifferenceSimple(b *testing.B) {
	if testing.Short() {
		b.Skip("skipping benchmark in short mode.")
	}
	for i := 0; i < b.N; i++ {
		DifferenceSimple(100)
	}
}
